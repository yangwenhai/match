...
func MakeGRPCServer(ctx context.Context, endpoint endpoint.Endpoint, tracer stdopentracing.Tracer, logger log.Logger) pb.KitfwServer {
...
func (s *grpcServer) Process(ctx oldcontext.Context, req *pb.KitfwRequest) (*pb.KitfwReply, error) {
	_, res, err := s.process.ServeGRPC(ctx, req)
	if err != nil {
		return nil, err
	}
	return res.(*pb.KitfwReply), nil
}
